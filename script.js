...
const Nav = {
  ...
  init() {
    ...
    this._scrollY         = 0; // for iOS scroll-lock restore

    window.addEventListener('scroll', debounce(this.onScroll.bind(this), 10), { passive: true });
    this.onScroll();

    // Open via hamburger, close via X button
    this.menuToggle?.addEventListener('click', () => this.toggleMobile());
    $('#mobile-menu-close')?.addEventListener('click', () => this.closeMobile());

    // Close on link tap
    this.mobileLinks.forEach(link => {
      link.addEventListener('click', () => this.closeMobile());
      link.addEventListener('touchstart', () => { link.style.opacity = '0.7'; }, { passive: true });
      link.addEventListener('touchend',   () => { link.style.opacity = '';    }, { passive: true });
    });
  ...
  openMobile() {
    this.isOpen = true;
    // iOS scroll-lock: record position then fix body in place
    ...
    setTimeout(() => $('#mobile-menu-close')?.focus(), 380);
  },

  closeMobile() {
    this.isOpen = false;
    // iOS scroll-lock release: restore body and scroll position
  ...
  _syncMobileThemeLabel() {
    ...
    if (this.mobileThemeLabel) this.mobileThemeLabel.textContent = label;
    // Also update the button's aria-label
    if (this.mobileThemeBtn) {
      this.mobileThemeBtn.setAttribute('aria-label',
...
const ThemeToggle = {
  ...
  init() {
    ...
    // Sync mobile menu label now that the theme is settled
    // (Nav.init ran before ThemeToggle.init, so we call it here too)
    setTimeout(() => Nav._syncMobileThemeLabel?.(), 0);
  },
...
/* ────────────────────────────────────────────────────────────────────
   ...
   Reads the actual rendered header height (includes env safe-area-inset-top
   on iOS so sections never slip behind the nav on any device).
──────────────────────────────────────────────────────────────────── */
const SmoothScroll = {
  init() {
    document.addEventListener('click', (e) => {
      ...
      // Use live rendered height — respects safe-area-inset-top on iOS
